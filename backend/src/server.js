...
app.post('/api/ops/admin/teachers', authRequired, async (req, res) => {
  try {
    if (!requireRoles(req, res, ['school_admin'])) return;
    const { full_name, email, password = 'EduSecure@1234' } = req.body;
    if (!full_name || !email) {
...
// Parent dashboard: aggregate children, recent homework, notifications, delegates and counts
app.get('/api/ops/parent/dashboard', authRequired, async (req, res) => {
  try {
    if (!requireRoles(req, res, ['parent', 'delegate'])) return;

    // parent name
    const parentRow = await query(`SELECT full_name FROM users WHERE id = ? LIMIT 1`, [req.user.sub]);
    const parent_name = parentRow && parentRow[0] ? parentRow[0].full_name : null;
    ...
    // recent homework for linked children
    ...
    // recent notifications for this parent
    ...
    // delegates count
    const delegatesRow = await query(
      `SELECT COUNT(*) AS cnt FROM delegates WHERE parent_id = ? AND status = 'approved'`,
      [req.user.sub]
    );
    const active_delegate_count = delegatesRow && delegatesRow[0] ? delegatesRow[0].cnt : 0;

    const unreadRow = await query(`SELECT COUNT(*) AS cnt FROM notifications WHERE user_id = ? AND read_at IS NULL`, [req.user.sub]);
    const unread_notification_count = unreadRow && unreadRow[0] ? unreadRow[0].cnt : 0;

    return res.json({ parent_name, children, homework, notifications, active_delegate_count, unread_notification_count });
  } catch (error) {
    return res.status(500).json({ error: error.message });
  }
});
