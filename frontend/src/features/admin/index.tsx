...
import { localApi, type AdminAttendanceClass, type AuditEntry, type DelegateQueueItem, type LocalUser } from "@/lib/localApi";
...
export default function AdminDashboard() {
  ...
  const [auditFilter, setAuditFilter] = useState({ actorId: "", action: "", from: todayIso, to: todayIso, page: 1 });
  ...
  useEffect(() => {
    void loadAdminData();
  }, [profile?.school_id, attendanceDate, auditFilter.actorId, auditFilter.action, auditFilter.from, auditFilter.to, auditFilter.page]);
      ...
      await localApi.ops.admin.onboardLearner({
        ...
        parent_invite:
          wizard.parent_invite_email || wizard.parent_invite_phone
            ? { email: wizard.parent_invite_email || undefined, phone: wizard.parent_invite_phone || undefined }
            : null,
      });
      toast.success("Learner onboarded");
      setWizard({ child_full_name: "", class_name: "", grade: "", parent_id: "", parent_invite_email: "", parent_invite_phone: "" });
      await loadAdminData();
    } catch (error) {
  ...
  const exportAttendanceCsv = () => {
    ...
    for (const cls of attendanceClasses) {
      for (const learner of cls.learners) {
        rows.push([cls.class_name, learner.full_name, learner.status, learner.reason ?? "", learner.attendance_date]);
      }
    }
      ...
      <main className="mx-auto max-w-7xl space-y-6 px-6 py-8">
        <section className="panel p-5">
          ...
          <form onSubmit={onboardLearner} className="grid gap-3 md:grid-cols-3">
            ...
            <button type="submit" className="rounded-md bg-primary px-4 py-2 text-sm font-semibold uppercase tracking-wider text-primary-foreground">Create learner + link</button>
          </form>
        </section>
        ...
        <section className="panel p-5">
          <div className="mb-4 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <button type="button" onClick={exportAttendanceCsv} className="rounded-md border border-border bg-panel-elevated px-3 py-1.5 text-xs font-semibold uppercase tracking-wider"><Download className="mr-1 inline h-3.5 w-3.5" />CSV</button>
            </div>
          </div>
          <div className="mb-2 text-xs text-muted-foreground">Present {attendanceTotals.present} / {attendanceTotals.total} · Absent {attendanceTotals.absent}</div>
        ...
        <section className="panel p-5">
          ...
          <div className="space-y-2">
            {delegates.length === 0 && <div className="text-sm text-muted-foreground">No pending delegates.</div>}
            {delegates.map((delegate) => (
              <div key={delegate.id} className="flex items-center justify-between rounded-md border border-border bg-panel-elevated p-3">
                ...
                <div className="flex gap-2">
                  <button type="button" onClick={() => void decideDelegate(delegate.id, "approve")} className="rounded-md bg-success px-3 py-1.5 text-xs font-semibold uppercase text-success-foreground"><CheckCircle className="mr-1 inline h-3.5 w-3.5" />Approve</button>
                  <button type="button" onClick={() => void decideDelegate(delegate.id, "reject")} className="rounded-md bg-destructive px-3 py-1.5 text-xs font-semibold uppercase text-destructive-foreground"><XCircle className="mr-1 inline h-3.5 w-3.5" />Reject</button>
                </div>
              </div>
            ))}
          </div>
        ...
        <section className="grid gap-4 lg:grid-cols-2">
          <div className="panel p-5">
            ...
            <form onSubmit={freezeAccount} className="space-y-3">
              ...
              <button type="submit" className="rounded-md bg-destructive px-4 py-2 text-xs font-semibold uppercase tracking-wider text-destructive-foreground"><Lock className="mr-1 inline h-3.5 w-3.5" />Freeze account</button>
            </form>
          </div>
          ...
          <div className="panel p-5">
            ...
            <form onSubmit={createTeacher} className="mb-4 grid gap-2">
              ...
              <button type="submit" className="rounded-md bg-primary px-4 py-2 text-xs font-semibold uppercase tracking-wider text-primary-foreground">Create teacher</button>
            </form>
            <div className="max-h-48 space-y-2 overflow-auto pr-1 text-sm">
              {teachers.map((teacher) => (
                <div key={teacher.id} className="flex items-center justify-between rounded-md border border-border bg-panel-elevated p-2">
                  ...
                  <button type="button" onClick={() => void localApi.ops.admin.deactivateTeacher(teacher.id).then(loadAdminData)} className="rounded-md border border-border px-2 py-1 text-[11px] uppercase text-muted-foreground">Deactivate</button>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="panel p-5">
          <div className="mb-3 flex items-center justify-between">
            ...
            <button type="button" onClick={exportAuditCsv} className="rounded-md border border-border bg-panel-elevated px-3 py-1.5 text-xs font-semibold uppercase tracking-wider"><Download className="mr-1 inline h-3.5 w-3.5" />CSV</button>
          </div>
          <div className="mb-3 grid gap-2 md:grid-cols-4">
            <input className="rounded-md border border-border bg-input px-3 py-2 text-sm" placeholder="Actor ID" value={auditFilter.actorId} onChange={(e) => setAuditFilter((f) => ({ ...f, actorId: e.target.value, page: 1 }))} />
            <input className="rounded-md border border-border bg-input px-3 py-2 text-sm" placeholder="Action" value={auditFilter.action} onChange={(e) => setAuditFilter((f) => ({ ...f, action: e.target.value, page: 1 }))} />
            <input type="date" className="rounded-md border border-border bg-input px-3 py-2 text-sm" value={auditFilter.from} onChange={(e) => setAuditFilter((f) => ({ ...f, from: e.target.value, page: 1 }))} />
            <input type="date" className="rounded-md border border-border bg-input px-3 py-2 text-sm" value={auditFilter.to} onChange={(e) => setAuditFilter((f) => ({ ...f, to: e.target.value, page: 1 }))} />
          </div>
